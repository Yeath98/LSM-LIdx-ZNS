...
void MemoryManager::getValue(int level, int num, uint64_t offset, uint64_t length) {
    if(completeTable[level].find(num) != completeTable[level].end()) {
        completeTable[level][num].wait();
...
void MemoryManager::resetZone(unsigned zone_order, bool force) {
    ...
    if(rate == 1) {
        if(force) {
            zone_err("Evicting Zone loops infinitely, data size is larger than ssd capacity\n");
            exit(1);
        } else {
            return;
        }
        ...
        if(rate != 0.0) {
            ...
            for (uint64_t table = start_table, index = 0; table < end_table; table += DEFAULT_SSTABLE_SIZE) {
                if (index < meta.deleted_table.size() && meta.deleted_table[index] == table) {
                    ...
                    auto it = remappingTable[table];
                    if(completeTable[it.first].find(it.second) != completeTable[it.first].end()) {
                        completeTable[it.first][it.second].wait();
                        completeTable[it.first].erase(it.second);
                    }
            ...
            while (i < rewrite_table.size()) {
                ...
                for (table = to_meta.write_ptr; table < to_meta.end && i < rewrite_table.size();
                    ...
                    femu_write(femu, DEFAULT_SSTABLE_SIZE, table, nullptr, false);
                }
