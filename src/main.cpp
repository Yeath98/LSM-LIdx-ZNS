...
void saveSSTableKeysToFile(KVStore& store, const std::string& filename) {
    std::vector<std::tuple<uint64_t, uint64_t, int, int>> keys;
    store.extractSSTableKeys(keys);
    ...
    for (const auto& key : keys) {
        uint64_t minKey = std::get<0>(key);
        uint64_t maxKey = std::get<1>(key);
        int level = std::get<2>(key);
        int order = std::get<3>(key);
...
int main() {
    ...
    // 保存 SST 块信息到文件
    saveSSTableKeysToFile(test.store, "sst_keys.dat");
    //测试打印SST信息    
    test.store.printSSTablesInfo();
    // 加载 HTM 模型
    test.store.loadHTMModel("htm_model.pkl");
    return 0;
}
